...
{
	ZCached_Mapping_Table::ZCached_Mapping_Table(unsigned int capacity) : capacity(capacity)
	{
	}

	ZCached_Mapping_Table::~ZCached_Mapping_Table()
	{
		std::unordered_map<LPA_type, ZMTSlotType*> addressMap;
		std::list<std::pair<LPA_type, ZMTSlotType*>> lruList;
	...
	inline bool ZCached_Mapping_Table::Exists(const stream_id_type streamID, const LPA_type lpa)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpa);
		auto it = addressMap.find(key);
		if (it == addressMap.end()) {
			DEBUG("ZCached_Mapping_Table::Exists, Stream ID:" << streamID << ", LPA:" << lpa << ", MISS")
				return false;
		}
		if (it->second->Status != ZMTEntryStatus::VALID) {
			DEBUG("ZCached_Mapping_Table::Exists, Stream ID:" << streamID << ", LPA:" << lpa << ", MISS")
			return false;
		}

		DEBUG("ZCached_Mapping_Table::Exists, Stream ID:" << streamID << ", LPA:" << lpa << ", HIT")
		return true;
	}

	PPA_type ZCached_Mapping_Table::Retrieve_ppa(const stream_id_type streamID, const LPA_type lpn)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpn);
		auto it = addressMap.find(key);
		assert(it != addressMap.end());
		assert(it->second->Status == ZMTEntryStatus::VALID);
		lruList.splice(lruList.begin(), lruList, it->second->listPtr);
		
		return it->second->PPA;
	}

	page_status_type ZCached_Mapping_Table::Get_bitmap_vector_of_written_sectors(const stream_id_type streamID, const LPA_type lpn)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpn);
		auto it = addressMap.find(key);
		assert(it != addressMap.end());
	...
	void ZCached_Mapping_Table::Update_mapping_info(const stream_id_type streamID, const LPA_type lpa, const PPA_type ppa, const page_status_type pageWriteState)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpa);
		auto it = addressMap.find(key);
		assert(it != addressMap.end());
	...
	void ZCached_Mapping_Table::Insert_new_mapping_info(const stream_id_type streamID, const LPA_type lpa, const PPA_type ppa, const unsigned long long pageWriteState)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpa);
		auto it = addressMap.find(key);
		if (it == addressMap.end()) {
		...
		it->second->Status = ZMTEntryStatus::VALID;
		it->second->PPA = ppa;
		it->second->WrittenStateBitmap = pageWriteState;
	...
	bool ZCached_Mapping_Table::Is_slot_reserved_for_lpn_and_waiting(const stream_id_type streamID, const LPA_type lpn)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpn);
		auto it = addressMap.find(key);
		if (it != addressMap.end()) {
			if (it->second->Status == ZMTEntryStatus::WAITING) {
				return true;
			}
		}
	...
	inline bool ZCached_Mapping_Table::Check_free_slot_availability()
	{
		return addressMap.size() < capacity;
	}
	
	void ZCached_Mapping_Table::Reserve_slot_for_lpn(const stream_id_type streamID, const LPA_type lpn)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpn);
		...
		ZMTSlotType* cmtEnt = new ZMTSlotType();
		cmtEnt->Dirty = false;
		cmtEnt->Stream_id = streamID;
		lruList.push_front(std::pair<LPA_type, ZMTSlotType*>(key, cmtEnt));
		cmtEnt->Status = ZMTEntryStatus::WAITING;
		cmtEnt->listPtr = lruList.begin();
		addressMap[key] = cmtEnt;
	}

	ZMTSlotType ZCached_Mapping_Table::Evict_one_slot(LPA_type& lpa)
	{
		...
		ZMTSlotType evictedItem = *lruList.back().second;
	...
	bool ZCached_Mapping_Table::Is_dirty(const stream_id_type streamID, const LPA_type lpa)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpa);
		auto it = addressMap.find(key);
		if (it == addressMap.end())
	...
	void ZCached_Mapping_Table::Make_clean(const stream_id_type streamID, const LPA_type lpn)
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(streamID, lpn);
		auto it = addressMap.find(key);
		if (it == addressMap.end()) {
	...
	ZAddressMappingDomain::ZAddressMappingDomain(unsigned int cmt_capacity, unsigned int cmt_entry_size, unsigned int no_of_translation_entries_per_page,
		ZCached_Mapping_Table* CMT,
		...
		ZMT_entry_size(cmt_entry_size), Translation_entries_per_page(no_of_translation_entries_per_page), No_of_inserted_entries_in_preconditioning(0),
		PlaneAllocationScheme(PlaneAllocationScheme), Channel_no(channel_no), Chip_no(chip_no), Die_no(die_no), Plane_no(plane_no)
	{

		DEBUG("Create Domain - ZMT capacity:" << cmt_capacity << ", ZMT entry size:" << cmt_entry_size << ", Translation entries per page:" << no_of_translation_entries_per_page << ", Total physical sectors:" << total_physical_sectors_no << ", Total logical sectors:" << total_logical_sectors_no << ", Sectors per page:" << sectors_no_per_page)
		...
		GlobalMappingTable = new ZGMTEntryType[Total_logical_pages_no];
		...
		//If ZMT is NULL, then each address mapping domain should create its own ZMT
		if (CMT == NULL) {
			//Each flow (address mapping domain) has its own ZMT, so ZMT is create here in the constructor
			this->CMT = new ZCached_Mapping_Table(cmt_capacity);
		} else {
			//The entire ZMT space is shared among concurrently running flows (i.e., address mapping domains of all flow)
			this->CMT = CMT;
		}

		Total_translation_pages_no = MVPN_type(Total_logical_pages_no / Translation_entries_per_page);
		GlobalTranslationDirectory = new ZGTDEntryType[Total_translation_pages_no + 1];
	...
	ZAddressMappingDomain::~ZAddressMappingDomain()
	{
		delete CMT;
	...
	inline void ZAddressMappingDomain::Update_mapping_info(const bool ideal_mapping, const stream_id_type stream_id, const LPA_type lpa, const PPA_type ppa, const page_status_type page_status_bitmap)
	{
		if (ideal_mapping) {
	...
	inline page_status_type ZAddressMappingDomain::Get_page_status(const bool ideal_mapping, const stream_id_type stream_id, const LPA_type lpa)
	{
		if (ideal_mapping) {
	...
	inline PPA_type ZAddressMappingDomain::Get_ppa(const bool ideal_mapping, const stream_id_type stream_id, const LPA_type lpa)
	{
		if (ideal_mapping) {
	...
	inline PPA_type ZAddressMappingDomain::Get_ppa_for_preconditioning(const stream_id_type stream_id, const LPA_type lpa)
	{
		return GlobalMappingTable[lpa].PPA;
	}

	inline bool ZAddressMappingDomain::Mapping_entry_accessible(const bool ideal_mapping, const stream_id_type stream_id, const LPA_type lpa)
	{
		if (ideal_mapping) {
			return true;
		} else {
			...
			Block_no_per_plane, Page_no_per_block, SectorsPerPage, PageSizeInByte, Overprovisioning_ratio, sharing_mode, fold_large_addresses)
	{
		...
		DEBUG("Create Address_Mapping_Unit_Zone_Level::Address_Mapping_Unit_Zone_Level")
		_my_instance = this;
		domains = new ZAddressMappingDomain*[no_of_input_streams];
		...
		for (unsigned int domainID = 0; domainID < no_of_input_streams; domainID++) {
			...
			*/
			ZMT_entry_size = (unsigned int)std::ceil(((2 * std::log2(total_physical_pages_no)) + sector_no_per_page) / 8);
			...
			ZCached_Mapping_Table* sharedZMT = NULL;
			unsigned int per_stream_cmt_capacity = 0;
			cmt_capacity = cmt_capacity_in_byte / ZMT_entry_size;

			switch (sharing_mode) {
				case CMT_Sharing_Mode::SHARED:
					per_stream_cmt_capacity = cmt_capacity;
					sharedZMT = new ZCached_Mapping_Table(cmt_capacity);
					break;
			...
			DEBUG("domain " << domainID << "per_stream_cmt_capacity = " << cmt_capacity_in_byte << " bytes in total / " << ZMT_entry_size << " bytes per entry = " << per_stream_cmt_capacity << " entries");
			...
			domains[domainID] = new ZAddressMappingDomain(per_stream_cmt_capacity, ZMT_entry_size, no_of_translation_entries_per_page,
				sharedZMT,
	...
	{
		DEBUG("Address_Mapping_Unit_Zone_Level::Translate_lpa_to_ppa_and_dispatch(), size = " << transactionList.size());
	...
	*/
	bool Address_Mapping_Unit_Zone_Level::translate_lpa_to_ppa(stream_id_type streamID, NVM_Transaction_Flash* transaction)
	{
		...
		DEBUG("Address_Mapping_Unit_Zone_Level::translate_lpa_to_ppa(), PPA: " << ppa);

		if (transaction->Type == Transaction_Type::READ) {
			if (ppa == NO_PPA) {
				DEBUG("Address_Mapping_Unit_Zone_Level::translate_lpa_to_ppa(), online_create_entry_for_reads()");
		...
		DEBUG("Address_Mapping_Unit_Zone_Level::Convert_ppa_to_address: ppn " <<  transaction->PPA << "address = " << transaction->Address.ChannelID 
		...
		if (is_translation_page) {
			...
			if (!domains[transaction->Stream_id]->Mapping_entry_accessible(ideal_mapping_table, transaction->Stream_id, transaction->LPA)) {
				if (!domains[transaction->Stream_id]->CMT->Check_free_slot_availability()) {
					LPA_type evicted_lpa;
					ZMTSlotType evictedItem = domains[transaction->Stream_id]->CMT->Evict_one_slot(evicted_lpa);
			...
			if (domains[stream_id]->Mapping_entry_accessible(ideal_mapping_table, stream_id, transaction->LPA)) {
				...
				if (!domains[stream_id]->CMT->Check_free_slot_availability()) {
					LPA_type evicted_lpa;
					ZMTSlotType evictedItem = domains[stream_id]->CMT->Evict_one_slot(evicted_lpa);
					if (evictedItem.Dirty) {
						/* In order to eliminate possible race conditions for the requests that
						* will access the evicted lpa in the near future (before the translation
						* write finishes), MQSim updates GMT (the on flash mapping table) right
	...
	{
		ZAddressMappingDomain* domain = domains[stream_id];
	...
	void Address_Mapping_Unit_Zone_Level::allocate_plane_for_user_write(NVM_Transaction_Flash_WR* transaction)
	{
		...
		ZAddressMappingDomain* domain = domains[transaction->Stream_id];
	...
	bool Address_Mapping_Unit_Zone_Level::request_mapping_entry(const stream_id_type stream_id, const LPA_type lpa)
	{
		...
		if (domain->GlobalTranslationDirectory[mvpn].MPPN == NO_MPPN) {
			if (!domain->CMT->Check_free_slot_availability()) {
				LPA_type evicted_lpa;
				ZMTSlotType evictedItem = domain->CMT->Evict_one_slot(evicted_lpa);
				if (evictedItem.Dirty) {
					/* In order to eliminate possible race conditions for the requests that
					* will access the evicted lpa in the near future (before the translation
					* write finishes), MQSim updates GMT (the on flash mapping table) right

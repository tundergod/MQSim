...
#define MAKE_TABLE_INDEX(LPN,STREAM)
	...
	enum class ZMTEntryStatus {FREE, WAITING, VALID};

	struct ZGTDEntryType //Entry type for the Global Translation Directory
	{
		MPPN_type MPPN;
	...
	struct ZMTSlotType
	{
		...
		bool Dirty;
		ZMTEntryStatus Status;
		std::list<std::pair<LPA_type, ZMTSlotType*>>::iterator listPtr;//used for fast implementation of LRU
		stream_id_type Stream_id;
	};

	struct ZGMTEntryType//Entry type for the Global Mapping Table
	{
		PPA_type PPA;
	...
	class ZCached_Mapping_Table
	{
	public:
		ZCached_Mapping_Table(unsigned int capacity);
		~ZCached_Mapping_Table();
		...
		ZMTSlotType Evict_one_slot(LPA_type& lpa);
	...
	* However, ZMT is shared among concurrent streams in two ways: 1) each address mapping domain
	* shares the whole ZMT space with other domains, and 2) each address mapping domain has
	* its own share of ZMT (equal partitioning of ZMT space among concurrent streams).*/
	class ZAddressMappingDomain
	{
	public:
		ZAddressMappingDomain(unsigned int cmt_capacity, unsigned int cmt_entry_size, unsigned int no_of_translation_entries_per_page,
		...
		* It is always kept in volatile memory.*/
		ZGTDEntryType* GlobalTranslationDirectory;
		...
		* proposal. It is always stored in non-volatile flash memory.*/
		ZGMTEntryType* GlobalMappingTable;
	...
	{
		friend class GC_and_WL_Unit_Pone_Level;
	public:
		Address_Mapping_Unit_Zone_Level(const sim_object_id_type& id, FTL* ftl, NVM_PHY_ONFI* flash_controller, Flash_Block_Manager_Base* block_manager,
	...
	private:
		...
		ZAddressMappingDomain** domains;
		unsigned int ZMT_entry_size, GTD_entry_size;//In ZMT MQSim stores (lpn, ppn, page status bits) but in GTD it only stores (ppn, page status bits)
